...
| `npm run x402` in `signer/` | **16 checks / 0 failed** against the running server, paying on public chain 97 (23 Sep) | the paid handshake actually happens: `402` + `accepts[]` when unpaid, then real settlement (`settleTx 0x54b2d531…`, 114,728 gas) and split (`splitTx 0x0694ae2d…`, 75,931 gas) — **190,659 gas total = 0.0000190659 BNB per paid verification**, measured from the receipts, not estimated — a `REVOKED` verdict, a decodable `X-PAYMENT-RESPONSE`, and measured **on chain, not in the response**: issuer +900, platform +100, client −1000 while holding **0 BNB and sending 0 transactions** |
