...
| `scripts/check.js` | 53 checks (the run prints the current count — it grows with the watched set): document shape, signature, status lists, chain-derived bits, the index invariant, and bitstring determinism under re-render and under reordered input |
