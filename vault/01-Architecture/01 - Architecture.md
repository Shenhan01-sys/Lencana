...
| **Assessment policy** *(added 24 Sep)* | **the issuer**, publishing `web/src/manifest.ts` | a `CourseManifest` carries the rubric, weights, pass mark and the `criteria` text. `rubricHash` = keccak over the *policy only*, printed into `achievement.criteria`, so the platform cannot quietly rewrite the rules after a diploma exists. `score.ts` is deliberately policy-blind and `issue.js --score` was **removed** (D37) |
| **Assessment executor** *(added 24 Sep)* | the issuer's agent, `signer/src/judge.js` + `grade.js` | mechanical signs first (`INSUFFICIENT_EVIDENCE` refuses before any model call), then a model at `temperature: 0`. Fail-closed: a criterion the model skipped stops the run instead of being filled with 0. Proven able to **fail** a submission (`npm run judge`), with a measured 91–100 spread on a good answer (D41) |
| **Learning surface** *(added 22 Sep)* | content authored by an issuer, rendered by us | `web/src/courses/*` typed data + `web/src/lms.ts` templates: 2 courses · 24 lessons · **34 pages**. No UI framework, on purpose, so `verify.ts` stays DOM-free and probeable from Node. Progress lives in localStorage and is labelled **not evidence** (D34) |
| **Revenue split** *(added 24 Sep)* | `contracts/SettlementSplit.sol` (ours, live on chain 97) | an x402 settlement has exactly one `payTo`, so funds must land somewhere and then divide. Fixed platform share in bps (25% cap), **no debt ledger**, per-payment replay guard, and `platformBps` can only ever go **down** (D38) |
