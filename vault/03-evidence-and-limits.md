...
| `npm run check` in `signer/` | **41 checks / 0 failed** (19 Sep) against a live anvil fork, 8+ of them against chain state | An `OpenBadgeCredential` 3.0 is built and signed with `DataIntegrityProof` + `eddsa-rdfc-2022` — the cryptosuite the certification guide actually lists. Changing a score, pointing the proof at another key, or deleting the key from the issuer document each make verification fail. The status bits are read from `statusOf()` on the deployed resolver, and for every credential in the store the bit is checked **at the index the document itself claims** — the invariant that makes a status list mean anything |
