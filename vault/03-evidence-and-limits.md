...
| `npm run judge` in `signer/` | **PENILAI SAH** — kontrol negatif, 5/5 (24 Sep) | a fluent, long, empty answer scores **4/100** (`0/25, 0/25, 0/25, 0/15, 4/10`) — below the issuer's own pass mark — while the substantive answer scores **91–100 across runs** (mean 96,4); a too-short answer is refused *before* the model is called. This is the test that makes "the agent assesses" mean something: **a judge that cannot fail a student is not judging.** `qwen/qwen3.8-27b` is not the default — measured on both essays it gave **100 / 8**: it does fail an empty answer, but it saturates at the ceiling (no headroom between "good" and "perfect") and offers no `structured_outputs` |
| `npm run delegate -- --essay … --judge` | graded by model → issued on **public chain 97** (24 Sep): `tx 0xf62691df…` for a course credential, 316,384 gas, 8/8 read back | the first credential here whose number did not come from a human typing a CLI flag: the essay was graded against the issuer's published rubric, the composite came out 94/70, and the model's name plus the rubric version are printed into the document's `result.method` |
| `npm run judge-variance` | 5× the same essay, `temperature: 0`: substantive **91–100 (spread 9, mean 96,4)**, empty-answer **2–3**, separation **93,6 points**, and the pass/fail decision identical in all 5 runs | the honest wording is *"keputusannya stabil, angkanya punya rentang ±5"* — **not** "the grade is reproducible". A single number quoted without its range would overstate the precision of an LLM judge, which is exactly the standard we demand of issuers. Also measured: this account's quota is **8.000 tokens/minute**, so batch issuance hits `429`; `judge.js` now honours `retry-after` and waits (1s → 3s → 12s observed) instead of reporting a broken judge |
| ⚠️ what the model grade does NOT do yet | — | since 24 Sep the number *is* carried into the OB3.0 document (`result.method` names the model, `achievement.criteria` names the rubric version, `comment` lists per-criterion scores) — what is still missing: a **real learner** was never assessed (the essays are fixtures in `signer/fixtures/`), no second judge was asked to agree with the first, and the model grade is not checked against a human's grade on the same text |
| `forge test --evm-version cancun --fork-url <chain 97>` | **97 passed / 0 failed** (23 Sep) | the whole on-chain layer works against **BAS as actually deployed on chain 97** |
| `forge test --evm-version cancun --fork-url <chain 56>` | **97 passed / 0 failed** (23 Sep), **identical gas** | cross-check: the primitive is the same on mainnet; the result is not a state coincidence |
...
| ✅ ~~"sisi HTTP belum pernah jalan"~~ → **sudah**, 23–24 Sep | `npm run x402` menendang server sungguhan: tanpa `X-PAYMENT` → `402` + `accepts[]` skema `exact` (payTo dan asset dicocokkan); dengan pembayaran → settlement nyata di chain 97 (`settleTx 0x3486ff75…` 114.728 gas + `splitTx 0xb2f045d0…` 75.931 gas = **190.659 gas = 0,0000190659 BNB per settlement**, diukur dari receipt), laporan `verdict: REVOKED`, header `X-PAYMENT-RESPONSE` terdekode, dan **selisih saldo dibaca dari chain**: penerbit +900, platform +100, klien −1000 sambil memegang **BNB = 0 dan nol transaksi**, split menahan 0. Rutenya **batch**: satu pembayaran untuk `credentialHashes:[a,b]` → dua laporan dengan **verdict berbeda** (`REVOKED`,`VALID`) dari SATU settlement, pendapatan naik tepat satu harga → **20/20 hijau**. Lihat **D40**: yang dijual adalah batch, bukan datanya |
| ⚠️ batas jalur berbayar itu | — | Servernya **lokal**, belum punya URL publik; fasilitatornya **kita sendiri** (produksi: pihak ketiga); tokennya koin demo ber-`mint` terbuka. Yang boleh ditulis: *"genggaman x402 bekerja end-to-end terhadap chain 97"* — bukan *"ada orang yang sudah membayar kami"* |
