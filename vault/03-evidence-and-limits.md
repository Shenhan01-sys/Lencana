...
| `npm run check` in `signer/` | **36 checks / 0 failed** (19 Sep), 8 of them against live chain state | An `OpenBadgeCredential` 3.0 is built and signed with `DataIntegrityProof` + `eddsa-rdfc-2022` — the cryptosuite the certification guide actually lists. Changing a score, pointing the proof at another key, or deleting the key from the issuer document each make verification fail. The status bits are then read from `statusOf()` on the deployed resolver, so what a standard verifier sees for the revoked credential is *revoked* and for the delisted issuer's credential is *suspended*: the D30 distinction survives the trip out of our own system |
