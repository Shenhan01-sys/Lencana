...
| ✅ ~~"sisi HTTP belum pernah jalan"~~ → **sudah**, 23 Sep | `npm run x402` menendang server sungguhan: tanpa `X-PAYMENT` → `402` + `accepts[]` skema `exact` (payTo dan asset dicocokkan); dengan pembayaran → settlement nyata di chain 97 (`settleTx 0x54b2d531…` 114.728 gas, `splitTx 0x0694ae2d…` 75.931 gas → **190.659 gas = 0,0000190659 BNB per verifikasi berbayar**, diukur dari receipt), laporan `verdict: REVOKED`, header `X-PAYMENT-RESPONSE` yang bisa di-decode, dan **selisih saldo dibaca langsung dari chain**: penerbit +900, platform +100, klien −1000 sambil memegang **BNB = 0 dan nol transaksi**, split menahan 0. 16/16 hijau |
