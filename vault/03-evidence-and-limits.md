...
| `npm run judge` in `signer/` | **PENILAI SAH** — kontrol negatif, 5/5 (24 Sep) | a fluent, long, empty answer scores **4/100** (`0/25, 0/25, 0/25, 0/15, 4/10`) — below the issuer's own pass mark — while the substantive answer scores 95; a too-short answer is refused *before* the model is called. This is the test that makes "the agent assesses" mean something: **a judge that cannot fail a student is not judging.** `qwen/qwen3.8-27b` is not the default — measured on both essays it gave **100 / 8**: it does fail an empty answer, but it saturates at the ceiling (no headroom between "good" and "perfect") and offers no `structured_outputs` |
| `npm run delegate -- --essay … --judge` | graded by model → issued on **public chain 97** (24 Sep): `tx 0x284db43a…`, 370,143 gas paid by the platform, 8/8 read back | the first credential here whose number did not come from a human typing a CLI flag. `temperature: 0` still moved one point between calls (95 → 94), so the claim is "the model and its answer are recorded", never "the grade is bit-reproducible" |
| ⚠️ what the model grade does NOT do yet | — | the score is the *trigger* for issuing a lesson attestation; it is not carried inside the OB3.0 document or the split, and no real learner has been assessed — the essays tested are fixtures in `signer/fixtures/` |
