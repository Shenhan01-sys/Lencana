...
Of the 90: 38 resolver fork tests + 9 end-to-end fork tests + 21 offline artifact tests + 22 offline settlement-split tests. **The number to quote is whatever `forge test` prints today** — it is not maintained in prose anywhere else, and the count is not a claim about quality either way.
...
| ⚠️ "paid verification splits revenue on-chain" | `SettlementSplit` exists and its 22 tests pass, but they run against **mocks** — they prove the arithmetic, the replay guard, the one-way `platformBps` ratchet and the fail-everything-or-nothing ordering. They do **not** prove the flow, because nothing has ever settled into it: the x402 primitive lives in `_research/x402-bnb-poc/` (31 tests) and is not wired to this contract, and no deployment of `SettlementSplit` exists on any chain. Safe wording: *the revenue-split rule is written and tested as a contract; the paid path that would use it is not connected yet* |
