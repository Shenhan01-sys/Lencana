...
| [[00-Overview/10 - Project Detail (long form)]] written for a **"±68k character" field** | The form's "Project Detail" ceiling is **5,600 characters**. 62k characters of work were not wasted — the long page stays as the reference behind the submission text — but the *pasteable* artifact is now `10 - Project Detail (long form) - Copy.md` (5,448 chars, one sequence diagram, no vault paths, no internal ticket IDs — and no course inventory either, which the builder rejected as not telling the product's story). Rule learned: field limits get **measured from the form before** the prose is sized to them, and the condensed version drops everything that only means something inside our own workspace | discovered by the builder pasting it on 27 Sep; recorded in [[00-Overview/08 - Submission Copy]] |
